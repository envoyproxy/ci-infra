...
apt_install_pkgs () {
    apt-get update
    apt-get install --no-install-recommends -y "${APT_PKGS_AGENT[@]}" "$@"
}
...
install_bazel () {
    wget -qO - "https://github.com/bazelbuild/bazelisk/releases/download/v${BAZELISK_VERSION}/bazelisk-linux-${ARCH}" \
...
install_bazel_remote () {
    wget -O \
         ...
         "https://github.com/buchgr/bazel-remote/releases/download/v${BAZEL_REMOTE_VERSION}/bazel-remote-${BAZEL_REMOTE_VERSION}-linux-${FULL_ARCH}"
...
agent_setup_build () {
    ...
    apt_add_docker
    apt_add_skopeo
    apt_install_pkgs "${APT_PKGS_EXTRA[@]}" "${APT_PKGS_BUILD[@]}"
    install_bazel
    install_bazel_remote
    configure_docker
    _agent_setup_finalize
...
agent_setup_minimal () {
    _agent_setup_init
    apt_install_pkgs "${APT_PKGS_EXTRA[@]}"
    _agent_setup_finalize
}
