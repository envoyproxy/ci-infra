...
resource "aws_launch_template" "nano_pool" {
  ...
  iam_instance_profile {
    arn = aws_iam_instance_profile.asg_nano_init_iam_instance_profile.arn
  }
  ...
  user_data = base64encode(templatefile("${path.module}/init.sh.tpl", {
    ...
    instance_profile_arn = aws_iam_instance_profile.asg_nano_iam_instance_profile.arn
    role_name = aws_iam_role.asg_nano_iam_role.name
  }))
  vpc_security_group_ids = [aws_security_group.allow_ssh.id]
}

resource "aws_autoscaling_group" "nano_pool" {
  name = local.asg_name
  ...
  mixed_instances_policy {
    launch_template {
      launch_template_specification {
        launch_template_id = aws_launch_template.nano_pool.id
        version = "$Latest"
      }
